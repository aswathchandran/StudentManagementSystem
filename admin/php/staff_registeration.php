...
<body style="background-color:#ced6e0">
	...
	<form action="staff_registeration.php" method="post" class="login_form">
		<strong><label>Staff Id</label></strong>
        ...
        <?php
			if(isset($_POST["submit-button"]))
			{
				...
				{
					$query="select * from student_registeration WHERE username='$username'";
					...
					{
						$query="select * from student_registeration WHERE email='$email'";
						$query_run=mysqli_query($con,$query);
						if(mysqli_num_rows($query_run)>0)
						{
						...
						{
							$query="select * from student_registeration WHERE phonenumber='$phonenumber'";
							$query_run=mysqli_query($con,$query);
							if(mysqli_num_rows($query_run)>0)
							{
							...
							{
								//to inser into semester
								//$query="insert into semester(username) values('$username')";
								//$query_run=mysqli_query($con,$query);
								//<------end of semester--->
								$encrypted_password=md5($password);
								$query="insert into staff_registeration values('$username','$firstname','$lastname','$dept','$email','$phonenumber','$password')";	
								$query_run=mysqli_query($con,$query);
								if($query_run)
								{
